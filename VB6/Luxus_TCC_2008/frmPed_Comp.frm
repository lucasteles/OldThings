...
If .TextMatrix(c, 0) = mskCodpe Then
    ...
    .TextMatrix(c, 3) = Format(tabelas2!Preço, "R$#,##0.00;(R$#,##0.00)")
    .TextMatrix(c, 4) = Format(.TextMatrix(c, 2) * Replace(.TextMatrix(c, 3), "R$", ""), "R$#,##0.00;(R$#,##0.00)")
...
If tabelas2.State = adStateOpen Then tabelas2.Close
tabelas2.Open "select * from preços where Codpe=" & mskCodpe
.TextMatrix(linha, 3) = Format(tabelas2!Preço, "R$#,##0.00;(R$#,##0.00)")
.TextMatrix(linha, 4) = Format(.TextMatrix(linha, 2) * Replace(.TextMatrix(linha, 3), "R$", ""), "R$#,##0.00;(R$#,##0.00)")
If tabelas3.State = adStateOpen Then tabelas3.Close
tabelas3.Open "Peças", conecta, adOpenKeyset, adLockOptimistic
If tabelas3.State = adStateOpen Then tabelas3.Close
tabelas3.Open "select * from peças where codigo=" & mskCodpe
...
txtValor_Total = Format(total(2), "R$#,##0.00;(R$#,##0.00)")
cc = cc + 1
linha2 = linha2 + 1
Wend
p:
...
.TextMatrix(cont, 2) = Format(tabelas3!Preço, "R$#,##0.00;(R$#,##0.00)")
...
If .Rows = 2 Then
  ...
  GoTo Y
  Else
  txtValor_Total = Format(Replace(txtValor_Total, "R$", "") - Replace(.TextMatrix(.Rows - 1, 4), "R$", ""), "R$#,##0.00;(R$#,##0.00)")
Y:
  .Clear
  .FormatString = "Código da Peça |                    Nome| Quantidade   |Preco Unitario   |Sub Total         "
  .Rows = 2
...
Printer.Print "   Nota Fiscal nº "; mskNumPed
Printer.Print "   Código do Fornecedor - "; mskCodfor,
Printer.Print "   Nome do Fornecedor - "; txtNomeFor
...
Printer.Print " "; er; "- "; "Codigo-"; FlexItem.TextMatrix(er, 0); "   Nome-"; FlexItem.TextMatrix(er, 1); "   Quant-" & FlexItem.TextMatrix(er, 2) & "  " & "   Preço-" & FlexItem.TextMatrix(er, 3); "   Sub Total-"; FlexItem.TextMatrix(er, 4)
er = er + 1
...
DT = tabelas!data
Hora = tabelas!Hora
txtValor_Total = Format(tabelas!Valor_Total, "R$#,##0.00;(R$#,##0.00)")
Pago = tabelas!Pago
...
FlexItem.TextMatrix(c, 3) = Format(tabelas2!valor, "R$#,##0.00;(R$#,##0.00)")
FlexItem.TextMatrix(c, 2) = tabelas2!Quantidade
FlexItem.TextMatrix(c, 4) = Format(Replace(FlexItem.TextMatrix(c, 3), "R$", "") * Replace(FlexItem.TextMatrix(c, 2), "R$", ""), "R$#,##0.00;(R$#,##0.00)")
c = c + 1
tabelas2.MoveNext
